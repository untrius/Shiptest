...
/obj/structure/altar_of_gods/attackby(obj/item/attacking_item, mob/user, params)
	//If we can sac, we do nothing but the sacrifice instead of typical attackby behavior (IE damage the structure)
	if(sect_to_altar?.can_sacrifice(attacking_item, user))
		sect_to_altar.on_sacrifice(attacking_item, user)
		return TRUE
	. = ..()

	//everything below is assumed you're bibling it up
	if(!istype(attacking_item, /obj/item/storage/book/bible))
		return
	...
	for(var/am_i_holy_unchecked_type in GLOB.player_list)
		if(!isliving(am_i_holy_unchecked_type))
			continue
		var/mob/living/am_i_holy_living = am_i_holy_unchecked_type
...
/obj/structure/altar_of_gods/proc/generate_available_sects(mob/user) //eventually want to add sects you get from unlocking certain achievements
	. = list()
	for(var/sect_subtype in subtypesof(/datum/religion_sect))
		var/datum/religion_sect/not_a_real_instance_rs = sect_subtype
		if(initial(not_a_real_instance_rs.starter))
			. += list(initial(not_a_real_instance_rs.name) = sectsect_subtype
